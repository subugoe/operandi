...
workflow {
  ...
  main:
    ...
    /* NOTE: HAVING MULTIPLE CHANNELS IS NOT WORKING
       When the DONE.txt is created, the watch_input_ch gets closed 
       without submitting the DONE.txt to process step1. 
       This leads to a situation in which "step1_out" folder never 
       creates the "DONE.txt_s1.txt" file 
       which is the "until" condition of watch_step1_ch !!!
    */
    
    watch_input_ch = Channel.watchPath("${params.input_folder}/*.txt", "create")
                            .until { "${it}" == "${params.input_folder}/DONE.txt" }

    watch_step1_ch = Channel.watchPath("${params.step1_out}/*.txt", "create")
                            .until { "${it}" == "${params.step1_out}/DONE_s1.txt" }

    watch_step2_ch = Channel.watchPath("${params.step2_out}/*.txt", "create")
                            .until { "${it}" == "${params.step2_out}/DONE_s1.txt_s2.txt" }

    step1(watch_input_ch)
    step2(watch_step1_ch)
    step3(watch_step2_ch)
