...
async def db_create_processing_stats(institution_id: str, user_id: str) -> DBProcessingStatsTotal:
    ...
    db_processing_stats = DBProcessingStatsTotal(institution_id=institution_id, user_id=user_id)
    await db_processing_stats.save()
    return db_processing_stats

@call_sync
async def sync_db_create_processing_stats(institution_id: str, user_id: str) -> DBProcessingStatsTotal:
...
@call_sync
async def sync_db_get_processing_stats(user_id: str) -> DBProcessingStatsTotal:
    return await db_get_processing_stats(user_id)


async def db_increase_processing_stats(find_user_id: str, **kwargs) -> DBProcessingStatsTotal:
...
async def db_increase_processing_stats_with_handling(logger, find_user_id: str, **kwargs) -> DBProcessingStatsTotal:
    try:
...
@call_sync
async def sync_db_increase_processing_stats(find_user_id: str, **kwargs) -> DBProcessingStatsTotal:
